...
async function blobGet(key) {
  ...
  for (const url of urlsFor(c, key)) {
    try {
      ...
      if (res.ok) { const t = await res.text(); if (!t) return null; try { const v = JSON.parse(t); return (v && v.__deleted__) ? null : v; } catch { return null; } }
      last = res.status;
    } catch { last = -1; }
  }
...
// Mark a record deleted immediately (readers filter these out), then
// also attempt a hard delete. The tombstone makes the delete show up
// right away even before Netlify's delete propagates.
async function tombstone(key) {
  try { await blobSet(key, { __deleted__: true, at: Date.now() }); } catch {}
  await blobDelete(key);
}

const K = {
  room: n => 'room:' + n, fix: id => 'fix:' + id, pay: id => 'pay:' + id, rent: id => 'rent:' + id,
  settings: 'settings', roomIdx: 'idx:rooms', fixIdx: 'idx:fix', payIdx: 'idx:pays', rentIdx: 'idx:rents'
};
...
async function readByIndex(indexKey, keyFn, prefix) {
  // Collision-proof: enumerate real keys via the blob list (so parallel
  // adds never lose records). Deletes write a tombstone (value {__deleted__:true})
  // which we filter out here — reliable even before the delete propagates.
  const listed = await blobList(prefix);
  let ids;
  if (listed && listed.length) {
    ids = listed.map(k => k.slice(prefix.length));
  } else {
    ids = (await blobGet(indexKey)) || [];
  }
  const items = await Promise.all(ids.map(id => blobGet(keyFn(id)).catch(() => null)));
  return items.filter(x => x && !x.__deleted__);
}
...
async function getRents() { return readByIndex(K.rentIdx, K.rent, 'rent:'); }
...
export default async (req) => {
  ...
  try {
    ...
    if (action === 'load') {
      const [rooms, fix, pays, rents, settings] = await Promise.all([getRooms(), getFix(), getPays(), getRents(), getSettings()]);
      return json({ rooms: rooms.map(r => ({ ...publicRoom(r), hasPin: !!r.pin })), fix, pays, rents, settings });
    }
    if (action === 'saveRoom') {
      ...
      const rec = {
        ...
        // These belong to the physical room, so they survive edits AND vacancy:
        clean: existing ? (existing.clean !== false) : true,      // default clean
        service: existing ? (existing.service !== false) : true,  // default in-service
        noteLog: existing && Array.isArray(existing.noteLog) ? existing.noteLog : []
      };
    ...
    if (action === 'toggleClean') {
      const r = await blobGet(K.room(String(p.number)));
      if (!r) return json({ error: 'Room not found' }, 404);
      r.clean = !r.clean;
      await blobSet(K.room(r.number), r);
      return json({ ok: true, clean: r.clean });
    }

    if (action === 'toggleService') {
      const r = await blobGet(K.room(String(p.number)));
      if (!r) return json({ error: 'Room not found' }, 404);
      r.service = r.service === false ? true : false;
      await blobSet(K.room(r.number), r);
      return json({ ok: true, service: r.service });
    }
    ...
    if (action === 'addNote') {
      ...
      r.noteLog.unshift({ id: uid(), text, at: Date.now() });
      await blobSet(K.room(r.number), r);
      return json({ ok: true });
    }
    ...
    if (action === 'deleteNote') {
      const r = await blobGet(K.room(String(p.number)));
      if (!r) return json({ error: 'Room not found' }, 404);
      r.noteLog = (r.noteLog || []).filter(n => n.id !== p.noteId);
      await blobSet(K.room(r.number), r);
      return json({ ok: true });
    }
    if (action === 'deleteRoom') {
      const num = String(p.number);
      await tombstone(K.room(num)); await indexRemove(K.roomIdx, num);
      return json({ ok: true });
    }
    if (action === 'markPaid') {
      const r = await blobGet(K.room(String(p.number)));
    ...
    if (action === 'deleteFix') {
      await tombstone(K.fix(p.id)); await indexRemove(K.fixIdx, p.id);
      return json({ ok: true });
    }
    if (action === 'addPay') {
      const id = uid();
    ...
    if (action === 'deletePay') {
      await tombstone(K.pay(p.id)); await indexRemove(K.payIdx, p.id);
      return json({ ok: true });
    }

    if (action === 'collectRent') {
      const num = String(p.room || '').trim();
      const amount = Math.round(parseFloat(p.amount) * 100) / 100;
      if (!num) return json({ error: 'Room is required' }, 400);
      ...
      const rec = {
        ...
        kind: p.kind === 'nightly' ? 'nightly' : 'rent',   // rent vs nightly stay
        at,
        // store the calendar month key for easy monthly grouping (e.g. "2026-07")
        month: new Date(at).toISOString().slice(0, 7)
      };
      await blobSet(K.rent(id), rec);
      await indexAdd(K.rentIdx, id);
      // also nudge the room's paid-through forward if asked
      if (p.advance) {
        const r = await blobGet(K.room(num));
        if (r) { r.paidThru = addCycle(r.paidThru || r.dueDate, r.cycle); r.dueDate = addCycle(r.dueDate, r.cycle); await blobSet(K.room(num), r); }
      }
      return json({ ok: true, id });
    }

    if (action === 'deleteRent') {
      await tombstone(K.rent(p.id)); await indexRemove(K.rentIdx, p.id);
      return json({ ok: true });
    }
